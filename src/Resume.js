...
export default function Resume() {
  ...
  function gptifyWork() {
    setIsLoading(true)
    fetch('http://localhost:5555/resume/gpt/work?prompt='+ prompt)
    .then(resp => resp.json()).then(data=>{setWorkItems(data); setIsLoading(false)});
  }

  useEffect(() => {
    fetch('http://localhost:5555/resume/data/work', {
      method: 'GET',
      headers: {
  ...
  useEffect(() => {
    fetch('http://localhost:5555/resume/data/education', {
      method: 'GET',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
  ...
  return (
    ...
    <>
      ...
      <TabPanel value={value} index={0}>
        ...
        <Tooltip title={<div>Gptify: Ask chatgpt to rewrite my resume in a different style (other language, speaking like a pirate or making it sound stupid for example). <br/>Disclaimer: All outputs are generated by chatGPT, I am in no way responsible of the results, this is a just for fun feature</div>}><Fab sx={fabStyle} color="primary" onClick={()=>{setOpen(!isDrawerOpen)}}><PsychologyIcon style={{ fontSize: '50px' }} /></Fab></Tooltip>
